...
public class Bruja {
	...
	public void  lanzarHechizo(Hechizo h) throws HogwartsException {
	...
	public void mostrarUltimoHechizo() throws HogwartsException {
		if(totalHechizos == 0) {
			throw new HogwartsException("No existen hechizos realizados.");
			}
		else {
			Hechizo ultimo = historial[totalHechizos-1];
			ultimo.toString();
		}
	}
	...
	public void mostrarHechizosDeUnaFecha(LocalDate fecha) {
		...
		for(int i = 0; i < totalHechizos; i++) {
			if(historial[i].getFechaLanzamiento().equals(fecha)) {
				System.out.println(historial[i]);
			}
		}
	}

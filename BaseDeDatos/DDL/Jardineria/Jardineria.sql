...
ALTER TABLE PEDIDO ADD CONSTRAINT CHK_FECHA_PEDIDO CHECK (FECHA_PEDIDO < FECHA_ENTREGA);

ALTER TABLE PRODUCTO MODIFY DIMENSIONES INT;

ALTER TABLE PRODUCTO ADD CONSTRAINT CHK_DIMENSION CHECK (DIMENSIONES > 0);

ALTER TABLE CLIENTE ADD CONSTRAINT CHK_NOMBRE_CLIENTE CHECK (NOMBRE = INITCAP(NOMBRE));

ALTER TABLE OFICINA ADD CONSTRAINT CHK_TELEFONO_OFICINA CHECK (TELEFONO REGEXP '^[0-9]+$');

ALTER TABLE CLIENTE ADD CONSTRAINT CHK_TELEFONO_CLIENTE CHECK (TELEFONO REGEXP '^[0-9]+$');

ALTER TABLE PEDIDO ADD CONSTRAINT CHK_ESTADO CHECK (ESTADO IN('Enviado','En transito'));

ALTER TABLE OFICINA MODIFY CODIGO_POSTAL INT;

ALTER TABLE CLIENTE MODIFY CODIGO_POSTAL INT;

ALTER TABLE CLIENTE ADD CONSTRAINT CHK_NOMBRE_CLIENTE 
CHECK (SUBSTRING(NOMBRE_CLIENTE, 1, 1) = UPPER(SUBSTRING(NOMBRE_CLIENTE, 1, 1)));
